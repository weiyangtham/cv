...
print_section <- function(position_data, section_id){
  aux = position_data %>% 
    ...
    mutate(no_description = ifelse(description=="1",1,0)) %>% 
    ungroup() %>% 
    ...
    mutate(
      ...
      description_bullets = ifelse(is.na(descriptions),"",map_chr(descriptions, ~paste('-', ., collapse = '\n'))),
    ) %>% 
    strip_links_from_cols(c('title', 'description_bullets')) %>% 
    mutate_all(~ifelse(is.na(.), 'N/A', .))
    
    for(i in 1:nrow(aux)){
      if(aux$no_description[i]==0){
        print(aux[i,] %>% glue_data(
          "### {title}",
          "\n\n",
          "{loc}",
          "\n\n",
